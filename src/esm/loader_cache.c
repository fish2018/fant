...
yyjson_doc *esm_package_json_cache_read(ant_t *js, const char *pkg_json_path, bool *out_owned) {
  *out_owned = false;
  ...
  if (!st) {
    *out_owned = true;
    if (js && js->storage && ant_storage_path_is_virtual(pkg_json_path)) {
      uint8_t *data = NULL;
      size_t size = 0;
      if (ant_storage_read_file(js->storage, pkg_json_path, &data, &size) != ANT_STORAGE_OK)
        return NULL;
      yyjson_doc *doc = yyjson_read((const char *)data, size, 0);
  ...
  if (js && js->storage && ant_storage_path_is_virtual(pkg_json_path)) {
    ...
    size_t size = 0;
    if (ant_storage_read_file(js->storage, pkg_json_path, &data, &size) == ANT_STORAGE_OK) {
      entry->doc = yyjson_read((const char *)data, size, 0);
      ant_storage_free_data(js->storage, data);
    }
  } else {
