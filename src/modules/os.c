...
static ant_value_t os_loadavg(ant_t *js, ant_value_t *args, int nargs) {
  ...
  // Android's Bionic libc does not expose getloadavg(). The kernel still
  // publishes the same values through /proc/loadavg on normal devices.
  FILE *fp = fopen("/proc/loadavg", "r");
  double loadavg[3] = {0, 0, 0};
  int parsed = fp ? fscanf(fp, "%lf %lf %lf", &loadavg[0], &loadavg[1], &loadavg[2]) : 0;
  if (fp) fclose(fp);
  if (parsed == 3) {
