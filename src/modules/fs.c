...
static ant_value_t fs_stats_object_new(ant_t *js, const fs_stat_fields_t *fields);
static ant_value_t encode_data(ant_t *js, const char *data, size_t len, fs_encoding_t encoding);
static ant_value_t fs_read_to_uint8array(ant_t *js, const char *data, size_t len);
static ant_value_t create_dirent_object(
  ant_t *js, const char *name, size_t name_len, uv_dirent_type_t type
);

static bool fs_path_is_content_uri(const char *path) {
  return path && strncmp(path, "content://", 10) == 0;
}

/* While an embedder has selected a project Storage root, every relative path
 * belongs to that project. Absolute FILE_PATH paths below the selected root
 * and opaque ant-saf:// module identities are accepted by
 * ant_storage_relative_path(). Paths outside the project stay normal POSIX
 * paths. A raw content:// URI is never a filename and must be rejected. */
static bool fs_is_storage_path(ant_t *js, const char *path) {
  if (!js || !js->storage || !path) return false;
  if (fs_path_is_content_uri(path)) return true;
  char *relative = ant_storage_relative_path(js->storage, path);
  bool result = relative != NULL;
  free(relative);
  return result;
}

static bool fs_is_saf_storage_path(ant_t *js, const char *path) {
  return js && js->storage &&
    ant_storage_context_kind(js->storage) == ANT_STORAGE_SAF_TREE &&
    fs_is_storage_path(js, path);
}

static char *fs_posix_path(ant_t *js, const char *path) {
  if (!path) return NULL;
  if (js && js->storage &&
      ant_storage_context_kind(js->storage) == ANT_STORAGE_FILE_PATH &&
      fs_is_storage_path(js, path)) {
    return ant_storage_file_path(js->storage, path);
  }
  return strdup(path);
}

static ant_value_t fs_reject_content_uri(ant_t *js, const char *path) {
  return js_mkerr(
    js,
    "content:// URI is not a filesystem path; select it as a SAF_TREE StorageLocation"
  );
}

static ant_value_t fs_storage_unsupported(
  ant_t *js,
  ...
  return js_mkerr(
    js,
    "%s is unsupported for SAF_TREE storage%s%s",
    operation,
    path ? ": " : "",
    path ? path : ""
  );
...
static ant_value_t fs_normalize_path_value(ant_t *js, ant_value_t value) {
  if (vtype(value) != T_STR) return value;
  const char *path = js_getstr(js, value, NULL);
  if (!path) return js_mkerr(js, "Failed to get path string");
  if (fs_path_is_content_uri(path)) return fs_reject_content_uri(js, path);
  if (!js || !js->storage) return value;

  if (ant_storage_path_is_virtual(path) &&
      !ant_storage_relative_path(js->storage, path)) {
    return js_mkerr(js, "storage path belongs to a different StorageLocation");
  }
  if (ant_storage_context_kind(js->storage) == ANT_STORAGE_SAF_TREE &&
      (path[0] == '/' || path[0] == '\\')) {
    return js_mkerr(js, "absolute POSIX paths are unavailable for SAF_TREE storage");
  }

  char *normalized = ant_storage_virtual_path(js->storage, path);
  if (!normalized) {
    if (path[0] != '/' && path[0] != '\\')
      return js_mkerr(js, "path escapes the selected project StorageLocation");
    return value;
  }
  ant_value_t result = js_mkstr(js, normalized, strlen(normalized));
  free(normalized);
  return result;
}

static ant_value_t fs_storage_error(
  ant_t *js,
  ant_storage_error_t error,
  const char *operation,
  const char *path,
  const char *dest
) {
  const char *message = ant_storage_error_string(error);
  if (dest) {
    return js_mkerr(js, "%s: %s '%s' -> '%s'", operation, message, path, dest);
  }
  return js_mkerr(js, "%s: %s '%s'", operation, message, path);
}
...
} fs_storage_list_state_t;

static bool fs_storage_list_visitor(
  const char *name,
  bool is_directory,
  void *data
) {
  fs_storage_list_state_t *state = (fs_storage_list_state_t *)data;
  if (!state || !name) return false;
  if (state->with_file_types) {
    int type = is_directory ? UV_DIRENT_DIR : UV_DIRENT_FILE;
    js_arr_push(
      state->js,
      state->array,
      create_dirent_object(state->js, name, strlen(name), type)
    );
  } else {
    js_arr_push(state->js, state->array, js_mkstr(state->js, name, strlen(name)));
  }
  return true;
}

static ant_value_t fs_storage_stat_object(
  ant_t *js,
  uint64_t size,
  bool is_directory
) {
  return fs_stats_object_new(js, &(fs_stat_fields_t){
    .mode = is_directory ? S_IFDIR : S_IFREG,
    .size = (double)size,
  });
}

static ant_value_t fs_storage_read_sync(
  ant_t *js,
  const char *path,
  fs_encoding_t encoding
) {
  uint8_t *data = NULL;
  size_t size = 0;
  ant_storage_error_t error = ant_storage_read_file(js->storage, path, &data, &size);
  if (error != ANT_STORAGE_OK) return fs_storage_error(js, error, "read", path, NULL);
  ant_value_t result = encoding != FS_ENC_NONE
    ? encode_data(js, (const char *)data, size, encoding)
    : fs_read_to_uint8array(js, (const char *)data, size);
  ant_storage_free_data(js->storage, data);
  return result;
}

static ant_value_t fs_storage_read_bytes_sync(
  ant_t *js,
  const char *path
) {
  uint8_t *data = NULL;
  ...
  ant_storage_error_t error = ant_storage_read_file(js->storage, path, &data, &size);
  if (error != ANT_STORAGE_OK) return fs_storage_error(js, error, "read", path, NULL);
  ant_value_t result = js_mkstr(js, (const char *)data, size);
  ant_storage_free_data(js->storage, data);
  return result;
}

static ant_value_t fs_storage_write_sync(
  ant_t *js,
  const char *path,
  const uint8_t *data,
  size_t size,
  ...
  ant_storage_error_t error = ant_storage_write_file(
    js->storage, path, data, size, truncate
  );
  return error == ANT_STORAGE_OK
    ? js_mkundef()
    : fs_storage_error(js, error, "write", path, NULL);
}
...
static ant_value_t fs_coerce_file_url_path(ant_t *js, ant_value_t arg) {
  ...
  if (decoded) {
    path = fs_normalize_path_value(js, js_mkstr(js, decoded, strlen(decoded)));
    free(decoded);
  }}
...
static ant_value_t fs_coerce_path(ant_t *js, ant_value_t arg) {
  if (vtype(arg) == T_STR) return fs_normalize_path_value(js, arg);
  ...
  if (vtype(path) == T_STR) return fs_normalize_path_value(js, path);
...
static ant_value_t fs_rm_impl(ant_t *js, ant_value_t *args, int nargs, bool return_promise) {
  ...
  char *storage_path = strndup(path, path_len);
  if (!storage_path) {
    ant_value_t err = js_mkerr(js, "Out of memory");
    if (!return_promise) return err;
    js_reject_promise(js, promise, err);
    return promise;
  }
  ...
  if (fs_is_storage_path(js, storage_path)) {
    ant_storage_error_t storage_error = ant_storage_remove(
      js->storage, storage_path, recursive
    );
    free(storage_path);
    if (storage_error != ANT_STORAGE_OK && !(force && storage_error == ANT_STORAGE_NOT_FOUND)) {
      ant_value_t err = fs_storage_error(js, storage_error, "rm", path, NULL);
      if (!return_promise) return err;
      js_reject_promise(js, promise, err);
      return promise;
    }
...
static ant_value_t builtin_fs_readFileSync(ant_t *js, ant_value_t *args, int nargs) {
  ...
  if (fs_is_storage_path(js, path_cstr)) {
    fs_encoding_t enc = (nargs > 1) ? parse_encoding(js, args[1]) : FS_ENC_NONE;
    ant_value_t result = fs_storage_read_sync(js, path_cstr, enc);
    free(path_cstr);
    return result;
  }
...
static ant_value_t builtin_fs_readFile(ant_t *js, ant_value_t *args, int nargs) {
  ...
  char *storage_path = strndup(path, path_len);
  if (!storage_path) return js_mkerr(js, "Out of memory");
  if (fs_is_storage_path(js, storage_path)) {
    ant_value_t value = fs_storage_read_sync(
      js, storage_path, (nargs > 1) ? parse_encoding(js, args[1]) : FS_ENC_NONE
    );
    free(storage_path);
    return is_err(value) ? fs_rejected_promise(js, value) : fs_resolved_promise(js, value);
  }
  free(storage_path);
...
static ant_value_t builtin_fs_stream(ant_t *js, ant_value_t *args, int nargs) {
  ...
  if (!storage_path) return js_mkerr(js, "Out of memory");
  if (fs_is_storage_path(js, storage_path)) {
    ant_value_t value = fs_storage_read_bytes_sync(js, storage_path);
    free(storage_path);
    return is_err(value) ? fs_rejected_promise(js, value) : fs_resolved_promise(js, value);
  }
  free(storage_path);
...
static ant_value_t fs_write_file_sync_impl(
  ...
  if (fs_is_storage_path(js, path_cstr)) {
    ant_value_t result = fs_storage_write_sync(
      js, path_cstr, (const uint8_t *)write_data.data, write_data.len,
      strcmp(mode, "ab") != 0
    );
    free(path_cstr);
    return result;
...
static ant_value_t builtin_fs_copyFileSync(ant_t *js, ant_value_t *args, int nargs) {
  ...
  if (fs_is_storage_path(js, src_cstr) || fs_is_storage_path(js, dest_cstr)) {
    if (!fs_is_storage_path(js, src_cstr) || !fs_is_storage_path(js, dest_cstr)) {
      free(src_cstr); free(dest_cstr);
      return js_mkerr(js, "copyFileSync cannot mix SAF_TREE and FILE_PATH paths");
    }
    ant_storage_error_t error = ant_storage_copy(js->storage, src_cstr, dest_cstr);
    ant_value_t result = error == ANT_STORAGE_OK
      ? js_mkundef() : fs_storage_error(js, error, "copyfile", src_cstr, dest_cstr);
    free(src_cstr); free(dest_cstr);
    return result;
  }
...
static ant_value_t fs_copy_file_sync_impl(
  ...
  if (fs_is_storage_path(js, src_cstr) || fs_is_storage_path(js, dest_cstr)) {
    if (!fs_is_storage_path(js, src_cstr) || !fs_is_storage_path(js, dest_cstr))
      return js_mkerr(js, "%s cannot mix SAF_TREE and FILE_PATH paths", op_name);
    ant_storage_error_t error = ant_storage_copy(js->storage, src_cstr, dest_cstr);
    return error == ANT_STORAGE_OK
      ? js_mkundef() : fs_storage_error(js, error, op_name, src_cstr, dest_cstr);
  }
  struct stat src_st;
...
static ant_value_t fs_copy_path_sync_impl(
  ...
  if (fs_is_storage_path(js, src_cstr) || fs_is_storage_path(js, dest_cstr)) {
    ...
    if (is_callable(opts->filter)) {
      ...
      ant_value_t included = fs_call_value(js, opts->filter, js_mkundef(), filter_args, 2);
    ...
    bool exists = false;
    ant_storage_error_t error = ant_storage_stat(
      js->storage, src_cstr, &size, &is_directory, &exists
    );
    if (error != ANT_STORAGE_OK || !exists)
      return fs_storage_error(js, error == ANT_STORAGE_OK ? ANT_STORAGE_NOT_FOUND : error,
                              op_name, src_cstr, dest_cstr);
    if (is_directory && !opts->recursive)
      return js_mkerr(js, "%s() recursive option is required to copy directories", op_name);
    error = ant_storage_copy(js->storage, src_cstr, dest_cstr);
    return error == ANT_STORAGE_OK
      ? js_mkundef() : fs_storage_error(js, error, op_name, src_cstr, dest_cstr);
  }
  if (is_callable(opts->filter)) {
...
static ant_value_t builtin_fs_renameSync(ant_t *js, ant_value_t *args, int nargs) {
  ...
  if (fs_is_storage_path(js, old_cstr) || fs_is_storage_path(js, new_cstr)) {
    if (!fs_is_storage_path(js, old_cstr) || !fs_is_storage_path(js, new_cstr)) {
      free(old_cstr); free(new_cstr);
      return js_mkerr(js, "renameSync cannot mix SAF_TREE and FILE_PATH paths");
    }
    ant_storage_error_t error = ant_storage_rename(js->storage, old_cstr, new_cstr);
    ant_value_t value = error == ANT_STORAGE_OK
      ? js_mkundef() : fs_storage_error(js, error, "rename", old_cstr, new_cstr);
    free(old_cstr); free(new_cstr);
    return value;
  }
...
static ant_value_t builtin_fs_utimesSync(ant_t *js, ant_value_t *args, int nargs) {
  ...
  if (fs_is_storage_path(js, path_cstr)) {
    free(path_cstr);
    return js_mkerr(js, "utimes is unsupported for SAF_TREE storage");
  }
...
static ant_value_t builtin_fs_truncateSync(ant_t *js, ant_value_t *args, int nargs) {
  ...
  if (fs_is_storage_path(js, path_cstr)) {
    ...
    ant_storage_error_t error = ant_storage_read_file(
      js->storage, path_cstr, &current, &current_size
    );
    if (error != ANT_STORAGE_OK) {
      ant_value_t value = fs_storage_error(js, error, "truncate", path_cstr, NULL);
      free(path_cstr);
      return value;
    }
    if (len < 0 || (uint64_t)len > SIZE_MAX) {
      ant_storage_free_data(js->storage, current);
      free(path_cstr);
      return js_mkerr(js, "truncate length is invalid");
    }
    size_t next_size = (size_t)len;
    uint8_t *next = (uint8_t *)calloc(next_size ? next_size : 1u, 1u);
    if (!next) {
      ant_storage_free_data(js->storage, current);
      free(path_cstr);
      return js_mkerr(js, "Out of memory");
    }
    memcpy(next, current, current_size < next_size ? current_size : next_size);
    ant_storage_free_data(js->storage, current);
    error = ant_storage_write_file(js->storage, path_cstr, next, next_size, true);
    free(next);
    ant_value_t value = error == ANT_STORAGE_OK
      ? js_mkundef() : fs_storage_error(js, error, "truncate", path_cstr, NULL);
    free(path_cstr);
    return value;
  }
...
static ant_value_t builtin_fs_unlinkSync(ant_t *js, ant_value_t *args, int nargs) {
  ...
  if (fs_is_storage_path(js, path_cstr)) {
    ant_storage_error_t error = ant_storage_remove(js->storage, path_cstr, false);
    ant_value_t value = error == ANT_STORAGE_OK
      ? js_mkundef() : fs_storage_error(js, error, "unlink", path_cstr, NULL);
    free(path_cstr);
    return value;
  }
...
do_mkdir:
  ...
  if (fs_is_storage_path(js, path_cstr)) {
    ant_storage_error_t error = ant_storage_mkdirs(js->storage, path_cstr);
    ant_value_t value = error == ANT_STORAGE_OK
      ? js_mkundef() : fs_storage_error(js, error, "mkdir", path_cstr, NULL);
    free(path_cstr);
    return value;
  }
...
static ant_value_t builtin_fs_rmdirSync(ant_t *js, ant_value_t *args, int nargs) {
  ...
  if (fs_is_storage_path(js, path_cstr)) {
    ant_storage_error_t error = ant_storage_remove(js->storage, path_cstr, false);
    ant_value_t value = error == ANT_STORAGE_OK
      ? js_mkundef() : fs_storage_error(js, error, "rmdir", path_cstr, NULL);
    free(path_cstr);
    return value;
  }
...
static ant_value_t builtin_fs_statSync(ant_t *js, ant_value_t *args, int nargs) {
  ...
  if (fs_is_storage_path(js, path_cstr)) {
    ...
    bool exists = false;
    ant_storage_error_t error = ant_storage_stat(
      js->storage, path_cstr, &size, &is_directory, &exists
    );
    if (error != ANT_STORAGE_OK || !exists) {
      ant_value_t value = error == ANT_STORAGE_OK
        ? js_mkerr(js, "stat: not found '%s'", path_cstr)
        : fs_storage_error(js, error, "stat", path_cstr, NULL);
      free(path_cstr);
      return value;
    }
    ant_value_t value = fs_storage_stat_object(js, size, is_directory);
    free(path_cstr);
    return value;
  }
...
static ant_value_t builtin_fs_lstatSync(ant_t *js, ant_value_t *args, int nargs) {
  ...
  if (fs_is_storage_path(js, path_cstr)) {
    ...
    if (error != ANT_STORAGE_OK || !exists) {
      ant_value_t value = error == ANT_STORAGE_OK
        ? js_mkerr(js, "lstat: not found '%s'", path_cstr)
        : fs_storage_error(js, error, "lstat", path_cstr, NULL);
      free(path_cstr);
      return value;
    }
    ant_value_t value = fs_storage_stat_object(js, size, is_directory);
...
static ant_value_t builtin_fs_existsSync(ant_t *js, ant_value_t *args, int nargs) {
  ...
  if (fs_is_storage_path(js, path_cstr)) {
    ...
    bool is_directory = false;
    bool exists = false;
    ant_storage_stat(js->storage, path_cstr, &size, &is_directory, &exists);
    free(path_cstr);
...
static ant_value_t builtin_fs_readdirSync(ant_t *js, ant_value_t *args, int nargs) {
  ...
  if (fs_is_storage_path(js, path_cstr)) {
    ...
    fs_storage_list_state_t state = {
      .js = js,
      .array = arr,
      .with_file_types = with_file_types,
    };
    ant_storage_error_t error = ant_storage_list(
      js->storage, path_cstr, fs_storage_list_visitor, &state
    );
    free(path_cstr);
    if (error != ANT_STORAGE_OK) return fs_storage_error(js, error, "readdir", path, NULL);
    return arr;
  }
