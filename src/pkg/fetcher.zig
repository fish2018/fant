...
pub const CancelCallback = ?*const fn (user_data: ?*anyopaque) callconv(.c) bool;
...
pub const StreamHandler = struct {
    ...
    on_complete: *const fn (u16, ?[]const u8, ?*anyopaque) void,
...
const CONNECT_TIMEOUT_NS = 15 * 1_000_000_000;
const CLOSE_DRAIN_TIMEOUT_NS = 500 * 1_000_000;
const REQUEST_TIMEOUT_NS = 30 * 1_000_000_000;
const META_STALL_TIMEOUT_NS = 30 * 1_000_000_000;
const META_PROGRESS_LOG_NS = 1 * 1_000_000_000;
const MAX_TARBALL_REDIRECTS: u8 = 5;
...
const Http2Client = struct {
    ...
    pub fn deinit(self: *Http2Client) void {
        const needs_close = self.connected != 0 or self.connect_pending;
        self.closing = true;
        ...
        // A connector may still own a DNS request while `connected == 0`.
        // Freeing the client in that state leaves tlsuv's callback with a dangling
        // `client` pointer and can crash the Android process during a failed or
        // cancelled npm install. Ask tlsuv to cancel both pending and established
        // connections, then drain the loop until its close callback has run.
        if (needs_close) {
            self.close_complete = false;
            self.deferred_free = false;
            self.tls.data = self;
            const close_rc = tlsuv.tlsuv_stream_close(&self.tls, onStreamClose);
            if (close_rc != 0) {
                debug.log("fetcher: stream close failed: {d}", .{close_rc});
            }
            const close_start: u64 = @intCast(std.Io.Timestamp.now(io, .boot).toNanoseconds());
            while (!self.close_complete) {
                _ = uv.uv_run(self.loop, uv.RUN_NOWAIT);
                if (self.close_complete) break;
                const now: u64 = @intCast(std.Io.Timestamp.now(io, .boot).toNanoseconds());
                if (now - close_start > CLOSE_DRAIN_TIMEOUT_NS) {
                    // DNS cancellation and TLS shutdown are asynchronous. Do not free
                    // the stream while libuv may still call into it; onStreamClose will
                    // perform the final destruction once tlsuv has released the handle.
                    debug.log("fetcher: stream close timed out; deferring client free", .{});
    ...
    fn finishDeinit(self: *Http2Client) void {
        ...
        for (&self.requests) |*req| {
            if (req.stream_id != -1) {
                ...
                if (req.redirect_location) |location| self.allocator.free(location);
            }
        }
    ...
    fn onStreamClose(handle: *uv.handle_t) callconv(.c) void {
        ...
        client.close_complete = true;
        if (client.deferred_free) client.finishDeinit();
    }
        ...
        if (frame.hd.flags & nghttp2.FLAG_END_STREAM != 0) {
            if (client.findRequest(frame.hd.stream_id)) |req| {
                if (!req.done) {
                    ...
                    if (req.on_complete) |cb| cb(req.status_code, req.redirect_location, req.userdata);
                }
            }
        }
        return 0;
    ...
    fn h2DataChunk(session: ?*nghttp2.session, _: u8, stream_id: i32, data: [*c]const u8, len: usize, ud: ?*anyopaque) callconv(.c) c_int {
        ...
        if (req.on_data) |cb| {
            // Streaming consumers expect the successful payload format. In
            // particular, npm registries commonly return a short HTML body with a
            // 302 tarball redirect. Never feed that body into the gzip extractor.
            if (req.status_code == 200) cb(data[0..len], req.userdata);
        } else req.response_body.appendSlice(client.allocator, data[0..len]) catch {
            req.has_error = true;
        };
        req.bytes += len;
    ...
    fn h2Header(_: ?*nghttp2.session, frame: *const nghttp2.frame, name: [*c]const u8, namelen: usize, value: [*c]const u8, valuelen: usize, _: u8, ud: ?*anyopaque) callconv(.c) c_int {
        ...
        if (std.mem.eql(u8, name[0..namelen], "location")) {
            if (req.redirect_location) |location| client.allocator.free(location);
            req.redirect_location = client.allocator.dupe(u8, value[0..valuelen]) catch null;
            if (req.redirect_location == null) req.has_error = true;
        }
        return 0;
    }
    ...
    fn h2StreamClose(_: ?*nghttp2.session, stream_id: i32, error_code: u32, ud: ?*anyopaque) callconv(.c) c_int {
        ...
        if (!req.done) {
            ...
            } else if (req.on_complete) |cb| cb(req.status_code, req.redirect_location, req.userdata);
        }
        return 0;
    }
    ...
    fn onRead(stream: *uv.stream_t, nread: isize, buf: *const uv.buf_t) callconv(.c) void {
        ...
        if (nread > 0 and client.h2_session != null) {
            const parsed = nghttp2.nghttp2_session_mem_recv(
                client.h2_session.?,
                @ptrCast(buf.base),
                @intCast(nread),
            );
            if (parsed < 0) {
                debug.log("fetcher: HTTP/2 parser failed code={d}", .{parsed});
                for (client.requests[0..client.request_count]) |*req| if (!req.done) {
                    req.done = true;
                    req.has_error = true;
                    req.end_ns = @intCast(std.Io.Timestamp.now(io, .boot).toNanoseconds());
                ...
                _ = tlsuv.tlsuv_stream_close(&client.tls, onStreamClose);
                return;
            }
            client.flush() catch {};
        }
    ...
    fn failPendingRequests(self: *Http2Client, err: FetchError) void {
        const now: u64 = @intCast(std.Io.Timestamp.now(io, .boot).toNanoseconds());
        for (self.requests[0..self.request_count]) |*req| if (!req.done) {
            req.done = true;
            req.has_error = true;
            req.end_ns = now;
            self.requests_done += 1;
            if (req.on_error) |cb| cb(err, req.userdata);
        };
    }
    ...
    fn onConnect(req: *uv.connect_t, status: c_int) callconv(.c) void {
        ...
        const protocol = tlsuv.tlsuv_stream_get_protocol(&ctx.client.tls);
        if (protocol == null or !std.mem.eql(u8, std.mem.span(protocol.?), "h2")) {
            debug.log("fetcher: TLS connected with unsupported ALPN protocol={s}", .{
                if (protocol) |value| std.mem.span(value) else "<none>",
            });
            ctx.client.connected = -1;
            _ = tlsuv.tlsuv_stream_close(&ctx.client.tls, onStreamClose);
            return;
        }
        debug.log("fetcher: TLS connected with ALPN h2", .{});
        ctx.client.connected = 1;
    ...
    fn ensureConnected(self: *Http2Client) !void {
        ...
        const connect_start: u64 = @intCast(std.Io.Timestamp.now(io, .boot).toNanoseconds());
        while (self.connected == 0) {
            if (self.cancel_callback) |callback| {
                if (callback(self.cancel_user_data)) {
                    return error.Cancelled;
                }
            }
            const now: u64 = @intCast(std.Io.Timestamp.now(io, .boot).toNanoseconds());
            if (now - connect_start > CONNECT_TIMEOUT_NS) return error.ConnectionFailed;
            _ = uv.uv_run(self.loop, uv.RUN_NOWAIT);
            _ = io.sleep(.fromNanoseconds(1 * 1_000_000), .boot) catch {};
            loop_count += 1;
        }
    ...
    fn makeNv(name: [:0]const u8, value: [:0]const u8) nghttp2.nv {
        return .{ .name = @constCast(name.ptr), .value = @constCast(value.ptr), .namelen = name.len, .valuelen = value.len, .flags = nghttp2.NV_FLAG_NONE };
    }
    ...
    pub fn getWithAccept(self: *Http2Client, path: []const u8, accept: [:0]const u8, allocator: std.mem.Allocator) ![]u8 {
        ...
        var hdrs = [_]nghttp2.nv{ makeNv(":method", "GET"), makeNv(":path", req.path.?), makeNv(":scheme", "https"), makeNv(":authority", self.host), makeNv("accept", accept), makeNv("user-agent", user_agent) };
        ...
        while (!req.done) {
            if (self.cancel_callback) |callback| {
                if (callback(self.cancel_user_data)) return error.Cancelled;
            }
            _ = uv.uv_run(self.loop, uv.RUN_NOWAIT);
            _ = io.sleep(.fromNanoseconds(1 * 1_000_000), .boot) catch {};
            try self.flush();
            const now: u64 = @intCast(std.Io.Timestamp.now(io, .boot).toNanoseconds());
            if (now - req.start_ns > REQUEST_TIMEOUT_NS) {
                debug.log("fetcher: HTTP/2 request timed out path={s}", .{path});
                req.done = true;
                req.has_error = true;
                req.end_ns = now;
    ...
    pub fn getStream(self: *Http2Client, path: []const u8, on_data: *const fn ([]const u8, ?*anyopaque) void, on_complete: *const fn (u16, ?[]const u8, ?*anyopaque) void, on_error: *const fn (FetchError, ?*anyopaque) void, userdata: ?*anyopaque) !void {
        try self.ensureConnected();
        const req = self.findOrAllocSlot() orelse return error.OutOfMemory;

        req.* = .{ .stream_id = 0, .path = try self.allocator.dupeZ(u8, path), .on_data = on_data, .on_complete = on_complete, .on_error = on_error, .userdata = userdata, .response_body = .empty, .status_code = 0, .done = false, .has_error = false, .start_ns = @intCast(std.Io.Timestamp.now(io, .boot).toNanoseconds()), .end_ns = 0, .bytes = 0, .content_encoding = .identity, .redirect_location = null };

        const session = self.h2_session orelse return error.Http2Error;

        var hdrs = [_]nghttp2.nv{ makeNv(":method", "GET"), makeNv(":path", req.path.?), makeNv(":scheme", "https"), makeNv(":authority", self.host), makeNv("accept", "*/*"), makeNv("user-agent", user_agent) };
    ...
    pub fn run(self: *Http2Client) !void {
        ...
        while (self.requests_done < self.request_count) {
            ...
            if (uv.uv_run(self.loop, uv.RUN_NOWAIT) == 0) break;
            _ = io.sleep(.fromNanoseconds(1 * 1_000_000), .boot) catch {};
            try self.flush();
            loop_count += 1;
...
const TarballCallbacks = struct {
    ...
    fn onComplete(status: u16, location: ?[]const u8, ud: ?*anyopaque) void {
        const ctx: *TarballCtx = @ptrCast(@alignCast(ud));
        if (isRedirectStatus(status)) {
            const redirect = location orelse {
                ctx.handler.on_error(FetchError.ResponseError, ctx.handler.user_data);
                ctx.done = true;
                ctx.has_error = true;
                return;
            };
            if (ctx.redirect_count >= MAX_TARBALL_REDIRECTS) {
                ctx.handler.on_error(FetchError.ResponseError, ctx.handler.user_data);
                ctx.done = true;
                ctx.has_error = true;
                return;
            }
            const next_url = resolveRedirectUrl(ctx.fetcher.allocator, ctx.url, redirect) catch {
                ctx.handler.on_error(FetchError.InvalidUrl, ctx.handler.user_data);
                ctx.done = true;
                ctx.has_error = true;
                return;
            };
            ctx.fetcher.pending.append(ctx.fetcher.allocator, .{
                .url = next_url,
                .handler = ctx.handler,
                .redirect_count = ctx.redirect_count + 1,
            }) catch {
                ctx.fetcher.allocator.free(next_url);
                ctx.handler.on_error(FetchError.OutOfMemory, ctx.handler.user_data);
                ctx.has_error = true;
            };
            ctx.done = true;
            return;
        }
        ctx.handler.on_complete(status, null, ctx.handler.user_data);
        ctx.done = true;
    }
...
fn resolveRedirectUrl(
    ...
    const source = try ParsedUrl.parse(source_url);
    if (!std.mem.eql(u8, source.scheme, "https")) return error.InvalidUrl;
    if (std.mem.startsWith(u8, location, "//"))
        return std.fmt.allocPrint(allocator, "https:{s}", .{location});

    const authority = if (source.port == 443)
        try allocator.dupe(u8, source.host)
    else
        try std.fmt.allocPrint(allocator, "{s}:{d}", .{ source.host, source.port });
    defer allocator.free(authority);

    if (std.mem.startsWith(u8, location, "/"))
        return std.fmt.allocPrint(allocator, "https://{s}{s}", .{ authority, location });

    const slash = std.mem.lastIndexOfScalar(u8, source.path, '/') orelse 0;
    return std.fmt.allocPrint(
        allocator,
        "https://{s}{s}{s}",
        .{ authority, source.path[0 .. slash + 1], location },
    );
}
...
pub const Fetcher = struct {
    ...
    // The backing array stays fixed for predictable allocation, while this
    // limit lets embedders reduce concurrent network work on constrained hosts.
    ...
    fn metaConnectionLimit(self: *const Fetcher) usize {
        return @min(self.connection_limit, NUM_META_CONNECTIONS);
    }

    pub fn init(allocator: std.mem.Allocator, registry_host: []const u8) !*Fetcher {
        return initWithConnections(allocator, registry_host, NUM_CONNECTIONS);
    }

    pub fn initWithConnections(
        allocator: std.mem.Allocator,
        registry_host: []const u8,
        max_connections: u32,
    ) !*Fetcher {
        const f = try allocator.create(Fetcher);
        const requested = if (max_connections == 0)
            NUM_CONNECTIONS
        else
            @as(usize, @intCast(max_connections));
        const connection_limit = @min(requested, NUM_CONNECTIONS);
        f.* = .{
            .allocator = allocator,
    ...
    pub fn setCancellation(self: *Fetcher, callback: CancelCallback, user_data: ?*anyopaque) void {
        self.cancel_callback = callback;
        self.cancel_user_data = user_data;
        for (self.meta_clients) |maybe_client| if (maybe_client) |client| {
            self.configureClient(client);
        };
        for (self.tarball_clients) |maybe_client| if (maybe_client) |client| {
            self.configureClient(client);
        };
        for (self.extra_tarball_clients.items) |client| self.configureClient(client);
    }

    fn checkCancelled(self: *const Fetcher) !void {
        if (self.cancel_callback) |callback| {
            if (callback(self.cancel_user_data)) return error.Cancelled;
        }
    }

    fn configureClient(self: *const Fetcher, client: *Http2Client) void {
        client.cancel_callback = self.cancel_callback;
        client.cancel_user_data = self.cancel_user_data;
    }
    ...
    fn ensureMetaClients(self: *Fetcher) !void {
        ...
        for (self.meta_clients[0..self.metaConnectionLimit()], 0..) |*slot, i| {
        ...
        for (self.meta_clients[0..self.metaConnectionLimit()]) |slot| {
            if (slot != null) {
                any_connected = true;
                break;
            }
    ...
    fn ensureTarballClients(self: *Fetcher) !void {
        ...
        debug.log("fetcher: initializing {d} persistent connections", .{self.connection_limit});
        const init_start: u64 = @intCast(std.Io.Timestamp.now(io, .boot).toNanoseconds());

        for (self.tarball_clients[0..self.connection_limit], 0..) |*slot, i| {
            try self.checkCancelled();
            const client = Http2Client.init(self.allocator, self.registry_host, true) catch |err| {
        ...
        var any_connected = false;
        for (self.tarball_clients[0..self.connection_limit]) |slot| {
            if (slot != null) {
                any_connected = true;
                break;
            }
    ...
    fn findAvailableClient(self: *Fetcher) ?TarballClientSlot {
        ...
        while (attempts < self.connection_limit) : (attempts += 1) {
            const idx = (self.tarball_round_robin + attempts) % self.connection_limit;
            if (self.tarball_clients[idx]) |client| {
                if (client.hasCapacity()) return .{ .client = client, .idx = idx };
            }
        }
        return null;
    }
    ...
    fn findAvailableClientForHost(self: *Fetcher, host: []const u8) !?TarballClientSlot {
        ...
        for (self.extra_tarball_clients.items, 0..) |client, i| {
            if (clientHostMatches(client, host) and client.hasCapacity()) {
                return .{ .client = client, .idx = self.connection_limit + i };
            }
        }
        ...
        return .{ .client = client, .idx = self.connection_limit + self.extra_tarball_clients.items.len - 1 };
    }

    pub fn initiateTarballConnectionsAsync(self: *Fetcher) void {
        if (self.tarball_clients_initialized) return;
        debug.log("fetcher: initiating {d} tarball connections (async)", .{self.connection_limit});
    ...
    pub fn queueTarballAsync(self: *Fetcher, url: []const u8, handler: StreamHandler) !void {
        ...
        self.tarball_round_robin = (available.idx + 1) % self.connection_limit;
    }
    ...
    fn dispatchPending(self: *Fetcher) void {
        while (self.pending.items.len > 0) {
            ...
            self.dispatchRequest(available.client, req.url, req.redirect_count, handler) catch |err| {
    ...
    fn dispatchRequest(self: *Fetcher, client: *Http2Client, url: []const u8, redirect_count: u8, handler: StreamHandler) !void {
        const parsed = try ParsedUrl.parse(url);
        const ctx = try self.allocator.create(TarballCtx);
    ...
    fn errToFetchError(err: anyerror) FetchError {
        return switch (err) {
            ...
            error.Timeout => FetchError.Timeout,
            error.Cancelled => FetchError.Cancelled,
    ...
    pub fn finishTarballs(self: *Fetcher) !void {
        ...
        while (self.tarball_contexts.items.len > 0 or self.pending.items.len > 0) {
            ...
            var timed_out = false;
            for (self.tarball_contexts.items) |ctx| {
                if (!ctx.done and now - ctx.start_ns > REQUEST_TIMEOUT_NS) {
                    timed_out = true;
                    debug.log("fetcher: tarball request timed out url={s}", .{ctx.url});
                }
            }
            if (timed_out) {
                for (&self.tarball_clients) |maybe_client| {
                    if (maybe_client) |c| c.failPendingRequests(FetchError.Timeout);
                }
                for (self.extra_tarball_clients.items) |c| c.failPendingRequests(FetchError.Timeout);
            }
            if (now - last_report > 1_000_000_000) {
    ...
    fn nextMetaClient(self: *Fetcher, conn_idx: *usize) ?*Http2Client {
        const connection_limit = self.metaConnectionLimit();
        conn_idx.* %= connection_limit;
        var attempts: usize = 0;
        while (attempts < connection_limit) : (attempts += 1) {
            if (self.meta_clients[conn_idx.*]) |c| {
                if (metaClientCanQueue(c)) return c;
            }
            conn_idx.* = (conn_idx.* + 1) % connection_limit;
        }
        return null;
    }
    ...
    pub fn fetchMetadataDual(
        ...
        const loop = uv.uv_default_loop();
        while (true) {
            try primary.checkCancelled();
            try fallback.checkCancelled();
    ...
    pub fn fetchMetadataBatch(self: *Fetcher, names: []const []const u8, allocator: std.mem.Allocator) ![]MetadataResult {
        ...
        while (offset < names.len) {
            ...
            for (offset..end) |i| {
                ...
                req.stream_id = sid;
                queued += 1;
                conn_idx = (conn_idx + 1) % self.metaConnectionLimit();
            }
    ...
    pub const MetadataCallback = *const fn (name: []const u8, data: ?[]const u8, has_error: bool, userdata: ?*anyopaque) void;
