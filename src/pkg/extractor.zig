...
pub const GzipDecompressor = struct {
  ...
  pub fn init(allocator: std.mem.Allocator) !*GzipDecompressor {
    ...
    self.stream = bridge.ant_pkg_inflate_new() orelse {
  ...
  pub fn deinit(self: *GzipDecompressor) void {
    bridge.ant_pkg_inflate_free(self.stream);
  ...
  pub fn decompress(
    ...
    while (offset < input.len) {
      ...
      const status = bridge.ant_pkg_inflate_chunk(
        self.stream,
        input.ptr + offset,
        input.len - offset,
        &consumed,
      ...
      if (status < 0 or (consumed == 0 and produced == 0))
        return error.DecompressionFailed;
      offset += consumed;
...
// Streaming extractor for StorageLocation backends. It deliberately writes
// each archive entry through Ant's Storage Context, so SAF projects and
// caches are never materialized in an app-private POSIX directory.
pub const StorageExtractor = struct {
  allocator: std.mem.Allocator,
  ...
  pub fn init(
    ...
    const self = try allocator.create(StorageExtractor);
    errdefer allocator.destroy(self);
    try storage.mkdirs(context, output_path);
    const decompressor = try GzipDecompressor.init(allocator);
    errdefer decompressor.deinit();
    self.* = .{
      .allocator = allocator,
  ...
  pub fn deinit(self: *StorageExtractor) void {
    ...
    self.pending_symlinks.deinit(self.allocator);
    self.allocator.free(self.output_path);
    self.decompressor.deinit();
    self.allocator.destroy(self);
  }

  pub fn feedCompressed(self: *StorageExtractor, data: []const u8) !void {
    if (try self.decompressor.decompress(data, handleStorageDecompressed, self)) {
      self.gzip_finished = true;
    }
  }

  fn handleStorageDecompressed(data: []const u8, user_data: ?*anyopaque) !void {
    const self: *StorageExtractor = @ptrCast(@alignCast(user_data));
    try self.feedTar(data);
  }

  fn feedTar(self: *StorageExtractor, data: []const u8) !void {
    var remaining = data;
    while (remaining.len > 0) {
      const result = self.parser.feed(remaining);
      remaining = remaining[result.consumed..];
  ...
  fn fullPath(self: *StorageExtractor, relative: []const u8) ![]u8 {
    return storage.join(self.allocator, self.output_path, relative);
  }

  fn handleEntry(self: *StorageExtractor, entry: TarParser.Entry) !void {
    if (entry.path.len == 0) return;
    switch (entry.entry_type) {
      .directory => {
        const path = try self.fullPath(entry.path);
        defer self.allocator.free(path);
        try storage.mkdirs(self.context, path);
      },
      .file => try self.createFile(entry.path),
      .symlink => try self.queueSymlink(entry.path),
    }
  }
  ...
  fn normalizeSymlinkTarget(
    self: *StorageExtractor,
    entry_path: []const u8,
    target: []const u8,
  ) ![]u8 {
    if (target.len == 0 or target[0] == '/' or
        std.mem.indexOfScalar(u8, target, '\\') != null)
      return error.InvalidPath;
    for (target) |ch| if (ch == 0 or ch < 0x20) return error.InvalidPath;
    ...
    var base_iter = std.mem.splitScalar(u8, parent, '/');
    while (base_iter.next()) |component| {
      if (component.len > 0) try components.append(self.allocator, component);
    }
    ...
    if (components.items.len == 0) return error.InvalidPath;
    return std.mem.join(self.allocator, "/", components.items);
  }

  fn queueSymlink(self: *StorageExtractor, entry_path: []const u8) !void {
    const length = std.mem.indexOfScalar(u8, &self.parser.header.linkname, 0) orelse
      self.parser.header.linkname.len;
    const target = try self.normalizeSymlinkTarget(
      entry_path,
      self.parser.header.linkname[0..length],
    );
    errdefer self.allocator.free(target);
    const path = try self.allocator.dupe(u8, entry_path);
    errdefer self.allocator.free(path);
    try self.pending_symlinks.append(self.allocator, .{ .path = path, .target = target });
  }

  fn materializePendingSymlinks(self: *StorageExtractor) !void {
    for (self.pending_symlinks.items) |item| {
      const source = try self.fullPath(item.target);
      defer self.allocator.free(source);
      const destination = try self.fullPath(item.path);
      defer self.allocator.free(destination);
      storage.remove(self.context, destination, true) catch {};
      ...
      self.files_extracted += try storage.countFiles(
        self.context,
        destination,
        self.allocator,
      );
  ...
  pub fn stats(self: *const StorageExtractor) struct { files: u32, bytes: u64 } {
    return .{ .files = self.files_extracted, .bytes = self.bytes_extracted };
  }

  pub fn isComplete(self: *const StorageExtractor) bool {
    return self.gzip_finished and self.archive_finished and self.files_extracted > 0;
  }
};
