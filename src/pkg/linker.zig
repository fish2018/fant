...
pub const StorageLinker = struct {
  ...
  destination: *const storage.Context,
  ...
  pub fn deinit(self: *StorageLinker) void {
    if (self.node_modules_path.len > 0) self.allocator.free(self.node_modules_path);
  }

  pub fn setNodeModulesPath(self: *StorageLinker, path: []const u8) !void {
    if (self.node_modules_path.len > 0) self.allocator.free(self.node_modules_path);
    self.node_modules_path = try self.allocator.dupe(u8, path);
    try storage.mkdirs(self.destination, path);
    const bin = try storage.join(self.allocator, path, ".bin");
    defer self.allocator.free(bin);
    try storage.mkdirs(self.destination, bin);
  }

  fn installPath(self: *StorageLinker, pkg: PackageLink) ![]u8 {
    const relative = if (pkg.parent_path) |parent|
    ...
    return storage.join(self.allocator, self.node_modules_path, relative);
  }
  ...
  fn readVersion(
    ...
    const path = storage.join(self.allocator, directory, "package.json") catch return null;
    defer self.allocator.free(path);
    const data = storage.read(context, path, self.allocator) catch return null;
    defer self.allocator.free(data);
    var doc = json.JsonDoc.parse(data) catch return null;
    defer doc.deinit();
    const version = doc.root().getString("version") orelse return null;
    return self.allocator.dupe(u8, version) catch null;
  }

  pub fn linkPackage(self: *StorageLinker, pkg: PackageLink) !void {
    const destination = try self.installPath(pkg);
    defer self.allocator.free(destination);
    const source_info = try storage.stat(self.source, pkg.cache_path);
    if (!source_info.exists or !source_info.is_directory) return error.PathNotFound;

    const source_version = self.readVersion(self.source, pkg.cache_path);
    defer if (source_version) |version| self.allocator.free(version);
    const destination_version = self.readVersion(self.destination, destination);
    defer if (destination_version) |version| self.allocator.free(version);
    if (source_version != null and destination_version != null and
        std.mem.eql(u8, source_version.?, destination_version.?) and
        (pkg.file_count == 0 or try storage.countFiles(
          self.destination,
          destination,
          self.allocator,
        ) >= pkg.file_count)) {
      self.stats.packages_skipped += 1;
      return;
    }

    storage.remove(self.destination, destination, true) catch {};
    try storage.copyTree(
      self.source,
      pkg.cache_path,
      self.destination,
      destination,
  ...
  fn writeBins(self: *StorageLinker, pkg: PackageLink) !void {
    for (pkg.bins) |bin| {
      if (!StorageLinker.binIsSafe(bin)) continue;
      const path = try std.fmt.allocPrint(
        self.allocator,
        "{s}/.bin/{s}",
        .{ self.node_modules_path, bin.name },
      );
      defer self.allocator.free(path);
      const target = try std.fmt.allocPrint(
        self.allocator,
        "../{s}/{s}",
        .{ pkg.name, if (std.mem.startsWith(u8, bin.path, "./")) bin.path[2..] else bin.path },
      );
      ...
      try storage.write(self.destination, path, script, true);
      self.stats.bins_linked += 1;
    }
  }

  fn binIsSafe(bin: PackageBin) bool {
    if (bin.name.len == 0 or bin.path.len == 0) return false;
    if (bin.name[0] == '/' or bin.path[0] == '/') return false;
    if (std.mem.indexOfScalar(u8, bin.name, '/') != null or
        std.mem.indexOfScalar(u8, bin.name, '\\') != null or
        std.mem.indexOf(u8, bin.path, "..") != null or
        std.mem.indexOfScalar(u8, bin.path, '\\') != null) return false;
    return true;
  }

  pub fn getStats(self: *const StorageLinker) StatsSnapshot {
    return self.stats;
  }
};
...
pub const Linker = struct {
  ...
  fn dirsAreCrossDevice(source_dir: std.Io.Dir, dest_dir: std.Io.Dir) bool {
    ...
    // Bionic did not export statx until API 30. Use the Linux syscall on
    // older Android targets while retaining the libc call where available.
    const statx = struct {
      fn call(fd: std.posix.fd_t, out: *std.os.linux.Statx, statx_flags: u32) bool {
        if (comptime std.c.versionCheck(.{ .major = 30, .minor = 0, .patch = 0 })) {
          return std.c.statx(fd, "", statx_flags, std.os.linux.STATX.BASIC_STATS, out) == 0;
        }
        const result = std.os.linux.statx(fd, "", statx_flags, std.os.linux.STATX.BASIC_STATS, out);
        return std.os.linux.errno(result) == .SUCCESS;
      }
    }.call;
    if (!statx(source_dir.handle, &source_stat, flags)) return false;
    if (!statx(dest_dir.handle, &dest_stat, flags)) return false;
