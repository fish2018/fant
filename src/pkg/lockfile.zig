...
pub const Lockfile = struct {
  data: []align(@alignOf(Header)) const u8,
  owned: bool,
  header: *const Header,
  ...
  pub fn openStorage(
    ...
    const raw = try storage.read(context, path, std.heap.c_allocator);
    defer std.heap.c_allocator.free(raw);
    if (raw.len < @sizeOf(Header)) return error.InvalidLockfile;

    const data = try std.heap.c_allocator.alignedAlloc(
      u8,
      std.mem.Alignment.fromByteUnits(@alignOf(Header)),
      raw.len,
    );
    errdefer std.heap.c_allocator.free(data);
    @memcpy(data, raw);
    var lock = try initFromOwnedData(data);
    lock.owned = true;
    return lock;
  }

  fn initFromOwnedData(data: []align(@alignOf(Header)) u8) !Lockfile {
    return initFromDataWindows(data);
  }
  ...
  pub fn close(self: *Lockfile) void {
    if (self.owned) std.heap.c_allocator.free(@constCast(self.data))
    else std.posix.munmap(@alignCast(self.data));
    self.* = undefined;
  }
...
pub const LockfileWriter = struct {
  ...
  pub fn serialize(self: *LockfileWriter) !Serialized {
    self.sortSideTables();
    ...
    try output.appendSlice(self.allocator, self.string_builder.items);
    
    if (package_pad_size > 0) {
      const padding = [_]u8{0} ** 8;
      try output.appendSlice(self.allocator, padding[0..package_pad_size]);
    } try output.appendSlice(self.allocator, std.mem.sliceAsBytes(self.packages.items));
    
    if (dep_pad_size > 0) {
      const padding = [_]u8{0} ** 8;
      try output.appendSlice(self.allocator, padding[0..dep_pad_size]);
    } try output.appendSlice(self.allocator, std.mem.sliceAsBytes(self.dependencies.items));
    
    if (hash_pad_size > 0) {
      const padding = [_]u8{0} ** 8;
      try output.appendSlice(self.allocator, padding[0..hash_pad_size]);
    } try output.appendSlice(self.allocator, std.mem.sliceAsBytes(hash_table));

    if (bin_pad_size > 0) {
      const padding = [_]u8{0} ** 8;
      try output.appendSlice(self.allocator, padding[0..bin_pad_size]);
    } try output.appendSlice(self.allocator, std.mem.sliceAsBytes(self.bin_entries.items));

    if (self.disabled_dependencies.items.len > 0 or self.platform_entries.items.len > 0) {
      if (disabled_pad_size > 0) {
        const padding = [_]u8{0} ** 8;
        try output.appendSlice(self.allocator, padding[0..disabled_pad_size]);
      }
    }

    if (self.disabled_dependencies.items.len > 0) {
      try output.appendSlice(self.allocator, std.mem.sliceAsBytes(self.disabled_dependencies.items));
    }

    if (self.platform_entries.items.len > 0) {
      if (platform_pad_size > 0) {
        const padding = [_]u8{0} ** 8;
        try output.appendSlice(self.allocator, padding[0..platform_pad_size]);
      } try output.appendSlice(self.allocator, std.mem.sliceAsBytes(self.platform_entries.items));
    }
  ...
  pub fn write(self: *LockfileWriter, path: []const u8) !u64 {
    ...
    try std.Io.Dir.cwd().writeFile(io, .{ .sub_path = path, .data = serialized.data });
    return serialized.graph_hash;
  }
  ...
  pub fn writeStorage(
    ...
    const serialized = try self.serialize();
    defer self.allocator.free(serialized.data);
    try storage.atomicWrite(context, path, serialized.data, self.allocator);
    return serialized.graph_hash;
  }
};
