...
fn readProjectData(
  ...
  return std.Io.Dir.cwd().readFileAlloc(
    io, path_z, allocator, .limited(10 * 1024 * 1024),
  );
}
...
export fn pkg_add_trusted_dependencies(
  ...
  const added = json.appendUniqueStringsToArrayFile(
    std.mem.span(package_json_path), "trustedDependencies", names.items,
...
fn portableResolveAndInstall(
    ...
    http.fetchTarball(package.tarball_url, fetcher.StreamHandler.init(
      struct {
        fn onData(data: []const u8, user_data: ?*anyopaque) void {
          ...
          context.ext.feedCompressed(data) catch {
            context.failed = true;
          };
        }
      }.onData,
  ...
  for (extract_contexts.items, 0..) |context, index| {
    ...
    if (context.failed or !context.completed or !context.ext.isComplete()) {
      ...
      c.setErrorFmt("Failed to download or extract {s}", .{context.package_name});
      return .extract_error;
    }
    const message = std.fmt.allocPrintSentinel(
      arena_alloc, "{s}", .{context.package_name}, 0,
    ) catch return .out_of_memory;
    c.reportProgress(
      .caching, @intCast(index + 1), @intCast(extract_contexts.items.len), message,
    );
  ...
  var package_linker = linker.StorageLinker.init(
    c.allocator, cache_context, project,
  );
  ...
  std.sort.heap(
    linker.PackageLink, links.items, {}, packageLinkLessThanParentFirst,
  );
  for (links.items, 0..) |link, index| {
    const message = std.fmt.allocPrintSentinel(
      arena_alloc, "{s}", .{link.name}, 0,
    ) catch return .out_of_memory;
  ...
  const graph_hash = res.writeLockfileStorage(
    project, lockfile_relative,
  ) catch |err| {
    c.setErrorFmt("Failed to write project ant.lockb: {}", .{err});
    return .io_error;
  };
  const marker_path = storage.join(
    arena_alloc, node_modules_relative, ".ant/install-state",
  ) catch return .out_of_memory;
  const marker = installStateMarkerContent(arena_alloc, graph_hash) catch
    return .out_of_memory;
