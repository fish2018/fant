...
// The portable installer links the current graph but must also remove direct
// packages left behind by an earlier graph. Nested packages are owned by their
// parent package and disappear with it; unchanged parents are immutable, so
// pruning the top-level install paths is sufficient and avoids deleting user
// files that are not package-manager outputs.
fn pruneStaleTopLevelPackages(
    project: *const storage.Context,
    ...
    while (resolved_iter.next()) |pkg_ptr| {
        const pkg = pkg_ptr.*;
        if (pkg.parent_path == null) {
            try expected.put(pkg.name.slice(), {});
        }
    }

    var removed: u32 = 0;
    const entries = try storage.list(project, node_modules_path, allocator);
    defer storage.freeEntries(allocator, entries);

    for (entries) |entry| {
        if (!entry.is_directory or
            std.mem.eql(u8, entry.name, ".bin") or
            std.mem.eql(u8, entry.name, ".ant")) continue;

        const package_path = try storage.join(allocator, node_modules_path, entry.name);
        defer allocator.free(package_path);

        if (entry.name.len > 0 and entry.name[0] == '@') {
            const scoped_entries = storage.list(project, package_path, allocator) catch continue;
            defer storage.freeEntries(allocator, scoped_entries);
            for (scoped_entries) |scoped| {
                if (!scoped.is_directory) continue;
                ...
                if (expected.contains(full_name)) continue;
                const stale_path = try storage.join(allocator, package_path, scoped.name);
                defer allocator.free(stale_path);
                try storage.remove(project, stale_path, true);
                removed += 1;
            }

            const remaining = storage.list(project, package_path, allocator) catch continue;
            defer storage.freeEntries(allocator, remaining);
            if (remaining.len == 0) {
                try storage.remove(project, package_path, true);
            }
            continue;
        }
...
fn pruneStaleTopLevelPackagesFs(
    node_modules_path: []const u8,
    res: *resolver.Resolver,
    allocator: std.mem.Allocator,
) !u32 {
    ...
    while (resolved_iter.next()) |pkg_ptr| {
        const pkg = pkg_ptr.*;
        if (pkg.parent_path == null) try expected.put(pkg.name.slice(), {});
    }

    var removed: u32 = 0;
    var node_modules = std.Io.Dir.cwd().openDir(io, node_modules_path, .{ .iterate = true }) catch
        return error.IoError;
    defer node_modules.close(io);
    var iter = node_modules.iterate();
    while (iter.next(io) catch null) |entry| {
        if (entry.kind != .directory or
            std.mem.eql(u8, entry.name, ".bin") or
            std.mem.eql(u8, entry.name, ".ant")) continue;
        if (entry.name.len > 0 and entry.name[0] == '@') {
            var scope = node_modules.openDir(io, entry.name, .{ .iterate = true }) catch continue;
            defer scope.close(io);
            var scope_iter = scope.iterate();
            while (scope_iter.next(io) catch null) |scoped| {
                if (scoped.kind != .directory) continue;
                const full_name = try std.fmt.allocPrint(
                    allocator,
                    "{s}/{s}",
                    .{ entry.name, scoped.name },
                ...
                const stale = try std.fmt.allocPrint(
                    allocator,
                    "{s}/{s}/{s}",
                    .{ node_modules_path, entry.name, scoped.name },
                );
                defer allocator.free(stale);
                std.Io.Dir.cwd().deleteTree(io, stale) catch |err| {
...
fn parseIntegrityHex(value: []const u8) ?[64]u8 {
    if (value.len != 128) return null;
    var result: [64]u8 = undefined;
    for (0..64) |index| {
        const high = switch (value[index * 2]) {
            '0'...'9' => value[index * 2] - '0',
            'a'...'f' => value[index * 2] - 'a' + 10,
            'A'...'F' => value[index * 2] - 'A' + 10,
            else => return null,
        };
        const low = switch (value[index * 2 + 1]) {
            '0'...'9' => value[index * 2 + 1] - '0',
            'a'...'f' => value[index * 2 + 1] - 'a' + 10,
            'A'...'F' => value[index * 2 + 1] - 'A' + 10,
            else => return null,
        };
...
fn pruneUnreferencedProjectCache(
    db: *cache.CacheDB,
    cache_context: *const storage.Context,
    res: *resolver.Resolver,
    allocator: std.mem.Allocator,
) !u32 {
    var expected = std.AutoHashMap([64]u8, void).init(allocator);
    defer expected.deinit();
    var resolved_iter = res.resolved.valueIterator();
    while (resolved_iter.next()) |pkg_ptr| {
        try expected.put(pkg_ptr.*.integrity, {});
    }

    const entries = try storage.list(cache_context, "entries", allocator);
    defer storage.freeEntries(allocator, entries);
    var removed: u32 = 0;
    for (entries) |entry| {
        if (entry.is_directory or !std.mem.endsWith(u8, entry.name, ".json")) continue;
        const hex = entry.name[0 .. entry.name.len - ".json".len];
        const integrity = parseIntegrityHex(hex) orelse continue;
        if (expected.contains(integrity)) continue;
        db.delete(&integrity) catch continue;
        removed += 1;
    }
    return removed;
}
...
pub const CancelCallback = ?*const fn (
    user_data: ?*anyopaque,
) callconv(.c) bool;

pub const StorageLocation = extern struct {
    kind: c_int = 0,
...
pub const PkgOptions = extern struct {
    ...
    cache_location: StorageLocation = .{},
    cache_bridge: ?*anyopaque = null,
    project_storage: ?*anyopaque = null,
    cache_storage: ?*anyopaque = null,
    cache_is_project_storage: bool = false,
    ...
    verbose: bool = false,
    force: bool = false,
    run_lifecycle_scripts: bool = false,
    cancel_callback: CancelCallback = null,
...
pub const PkgContext = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, options: PkgOptions) !*PkgContext {
        ...
        const default_cache_path = if (options.cache_dir == null and
            options.cache_storage == null)
        ...
        defer {
            if (default_cache_path) |path| allocator.free(path);
        }
        const cache_context = storage.fromOpaque(options.cache_storage);
        const cache_path = if (cache_context != null)
            (try storage.virtualPath(cache_context.?, "", allocator))
        else if (options.cache_dir) |dir| std.mem.span(dir) else default_cache_path.?;
        defer if (cache_context != null) allocator.free(cache_path);
        ...
        ctx.* = .{
            ...
            .last_install_result = .{ .package_count = 0, .cache_hits = 0, .cache_misses = 0, .files_linked = 0, .files_copied = 0, .packages_installed = 0, .packages_skipped = 0, .lifecycle_builds = 0, .elapsed_ms = 0 },
        ...
        ctx.cache_db = if (cache_context) |context| blk: {
            break :blk cache.CacheDB.openStorage(context) catch |err| {
                ctx.setErrorFmt("Failed to open portable cache: {}", .{err});
                return error.CacheError;
            };
        } else cache.CacheDB.open(ctx.cache_dir) catch |err| {
        ...
        ctx.http = fetcher.Fetcher.initWithConnections(
            allocator,
            registry,
        ...
        if (ctx.http) |http| {
            http.setCancellation(options.cancel_callback, options.cancel_user_data);
        }
    ...
    pub fn isCancelled(self: *const PkgContext) bool {
        if (self.options.cancel_callback) |callback| {
            return callback(self.options.cancel_user_data);
        }
        return false;
    }

    pub fn checkCancelled(self: *const PkgContext) !void {
        ...
        if (misses.items.len > 0) {
            ...
            for (misses.items, 0..) |pkg_idx, i| {
                ...
                http.fetchTarball(tarball_url, fetcher.StreamHandler.init(
                    ...
                    struct {
                        fn onComplete(status: u16, _: ?[]const u8, user_data: ?*anyopaque) void {
                            const ctx: *PkgExtractCtx = @ptrCast(@alignCast(user_data));
            ...
            for (extract_contexts[0..valid_count], 0..) |*ctx, i| {
                ...
                if (ctx.staging_path) |staging| {
                    const from_z = arena_alloc.dupeZ(u8, staging) catch {
                        error_count += 1;
                        std.Io.Dir.cwd().deleteTree(io, staging) catch {};
                        continue;
                    };
                    const to_z = arena_alloc.dupeZ(u8, ctx.cache_path) catch {
                        error_count += 1;
                        continue;
                    };
...
fn setResolveError(c: *PkgContext, target: ?[]const u8, err: anyerror) void {
    ...
    if (target) |name| c.setErrorFmt("Failed to resolve {s}: {}", .{ name, err }) else c.setErrorFmt("Failed to resolve dependencies: {}", .{err});
}
...
fn readProjectData(
    ...
    if (storage.fromOpaque(c.options.project_storage)) |context| {
        const relative = try storage.relativePath(context, path, allocator);
        defer allocator.free(relative);
        return storage.read(context, relative, allocator);
    }
...
fn writeProjectData(
    ...
    if (storage.fromOpaque(c.options.project_storage)) |context| {
        const relative = try storage.relativePath(context, path, allocator);
        defer allocator.free(relative);
        return storage.atomicWrite(context, relative, data, allocator);
    }
    const path_z = try allocator.dupeZ(u8, path);
    defer allocator.free(path_z);
    try std.Io.Dir.cwd().writeFile(io, .{ .sub_path = path_z, .data = data });
}
...
// The portable Android storage path cannot use std.Io.Dir to inspect the
// install marker. Keep a separate versioned marker format that also records
// the exact package.json bytes used for the install. This makes the startup
// fast path safe: formatting or dependency changes invalidate the marker and
// fall back to the normal resolver.
fn portableInstallStateMarkerContent(
    allocator: std.mem.Allocator,
    graph_hash: u64,
    package_json_hash: u64,
) ![]const u8 {
    return std.fmt.allocPrint(
        allocator,
        "ant-install-state-v2\n{x}\n{x}\n{s}\n{s}\n{s}\n",
        .{ graph_hash, package_json_hash, @tagName(builtin.os.tag), @tagName(builtin.cpu.arch), platform.abiName() },
    );
}

fn portableInstallStateMarkerMatches(
    allocator: std.mem.Allocator,
    project: *const storage.Context,
    node_modules_relative: []const u8,
    graph_hash: u64,
    package_json_hash: u64,
) bool {
    if (graph_hash == 0 or package_json_hash == 0) return false;
    const marker_path = storage.join(allocator, node_modules_relative, ".ant/install-state") catch return false;
    defer allocator.free(marker_path);
    const expected = portableInstallStateMarkerContent(allocator, graph_hash, package_json_hash) catch return false;
    defer allocator.free(expected);
    const actual = storage.read(project, marker_path, allocator) catch return false;
    defer allocator.free(actual);
    return std.mem.eql(u8, actual, expected);
}

fn portableLegacyInstallStateMarkerMatches(
    allocator: std.mem.Allocator,
    project: *const storage.Context,
    node_modules_relative: []const u8,
    graph_hash: u64,
...
export fn pkg_install(
    ...
    c.checkCancelled() catch |err| if (err == error.Cancelled) return .cancelled else return .io_error;
    ...
    if (c.options.run_lifecycle_scripts and pkg_json.trusted_dependencies.count() > 0) {
...
export fn pkg_add_trusted_dependencies(
    ...
    for (0..count) |i| {
        ...
        names.append(allocator, pkg_name) catch return .out_of_memory;
    }
    const added = json.appendUniqueStringsToArrayFile(
    ...
    debug.log("[trust] added {d} packages", .{added});
    return .ok;
}
        ...
        self.http.fetchTarball(pkg.tarball_url, fetcher.StreamHandler.init(
            struct {
                fn onData(data: []const u8, ud: ?*anyopaque) void {
                    ...
                    c.ext.feedCompressed(data) catch {
                        c.has_error = true;
                    };
                }
            }.onData,
            struct {
                fn onComplete(status: u16, _: ?[]const u8, ud: ?*anyopaque) void {
                    const c: *InterleavedExtractCtx = @ptrCast(@alignCast(ud));
...
const PortableExtractContext = struct {
    ext: *extractor.StorageExtractor,
    integrity: [64]u8,
    cache_path: []const u8,
    package_name: []const u8,
    version: []const u8,
...
fn portableResolveAndInstall(
    ...
    if (c.isCancelled()) {
        c.setError("Dependency installation was cancelled");
        return .cancelled;
    }
    _ = c.arena_state.reset(.retain_capacity);
    const arena_alloc = c.arena_state.allocator();
    ...
    if (c.options.run_lifecycle_scripts) {
        c.setError("Lifecycle scripts are not supported for StorageLocation projects");
        return .invalid_argument;
    }

    const package_json_relative = storage.relativePath(
        project,
        package_json_path,
        arena_alloc,
    ) catch {
        c.setError("package.json is outside the configured project location");
        return .invalid_argument;
    };
    const lockfile_relative = storage.relativePath(
        project,
        lockfile_path,
        arena_alloc,
    ) catch {
        c.setError("ant.lockb is outside the configured project location");
        return .invalid_argument;
    };
    const node_modules_relative = storage.relativePath(
        project,
        node_modules_path,
        arena_alloc,
    ) catch {
        c.setError("node_modules is outside the configured project location");
        return .invalid_argument;
    };

    const package_json_data = storage.read(
        project,
        package_json_relative,
        arena_alloc,
    ) catch |err| {
        c.setErrorFmt("Failed to read project package.json: {}", .{err});
        return .io_error;
    };
    var package_json = json.PackageJson.parseData(
        c.allocator,
        package_json_data,
    ) catch {
        c.setError("Failed to parse project package.json");
        return .invalid_argument;
    };
    defer package_json.deinit(c.allocator);
    c.reportProgress(.resolving, 0, 0, "package.json");

    // A successful portable install records the lockfile graph and the exact
    // package.json bytes in node_modules/.ant/install-state. On the very common
    // Start -> Stop -> Start path, validate that small marker before creating
    // the resolver or any HTTP clients. This avoids registry requests and all
    // SAF directory enumeration when dependencies are already installed.
    const package_json_hash = std.hash.Wyhash.hash(0, package_json_data);
    if (lockfile.Lockfile.openStorage(project, lockfile_relative)) |lock_value| {
        var existing_lock = lock_value;
        defer existing_lock.close();
        const registry = if (c.options.registry_url) |url|
            std.mem.span(url)
        else
            "https://registry.npmjs.org";
        const resolution_hash = resolutionCacheKey(arena_alloc, &package_json, registry) orelse 0;
        const current_marker = portableInstallStateMarkerMatches(
                arena_alloc,
                project,
                node_modules_relative,
                existing_lock.header.graph_hash,
                package_json_hash,
            );
        const legacy_marker = existing_lock.header.resolution_hash != 0 and
            existing_lock.header.resolution_hash == resolution_hash and
            portableLegacyInstallStateMarkerMatches(
                arena_alloc,
                project,
                node_modules_relative,
                existing_lock.header.graph_hash,
            );
        if (existing_lock.header.hasInstallMetadata() and
            existing_lock.header.matchesCurrentPlatform() and
            (current_marker or legacy_marker)) {
            const elapsed_ms = timer.untilNow(io).raw.toMilliseconds();
            c.last_install_result = .{
                .package_count = existing_lock.header.package_count,
                .cache_hits = existing_lock.header.package_count,
                .cache_misses = 0,
                .files_linked = 0,
                .files_copied = 0,
                .packages_installed = 0,
                .packages_skipped = existing_lock.header.package_count,
                .lifecycle_builds = 0,
                .elapsed_ms = @intCast(elapsed_ms),
            };
            c.reportProgress(.linking, existing_lock.header.package_count, existing_lock.header.package_count, "已验证现有依赖");
            debug.log("portable install fast path: marker and package.json match, packages={d} elapsed={d}ms", .{
                existing_lock.header.package_count,
                elapsed_ms,
            });
            return .ok;
        }
    } else |_| {
        // Missing or invalid ant.lockb: continue through the normal resolver.
    }
    ...
    defer res.deinit();
    res.force_refresh = c.options.force;
    res.setLockResolutionHash(
        resolutionCacheKey(arena_alloc, &package_json, registry) orelse 0,
    );
    res.resolveFromPackageJsonData(package_json_data) catch |err| {
    ...
    var extract_contexts = std.ArrayListUnmanaged(*PortableExtractContext).empty;
    defer extract_contexts.deinit(arena_alloc);
    var queued_integrities = std.AutoHashMap([64]u8, void).init(arena_alloc);
    defer queued_integrities.deinit();
    var cache_hits: u32 = 0;

    var package_iter = res.resolved.valueIterator();
    while (package_iter.next()) |package_ptr| {
        const package = package_ptr.*;
        if (package.direct) {
            const version = package.version.format(arena_alloc) catch return .out_of_memory;
            c.addPackageToResults(package.name.slice(), version, true) catch
                return .out_of_memory;
        }
        if (queued_integrities.contains(package.integrity)) continue;
        queued_integrities.put(package.integrity, {}) catch return .out_of_memory;

        if (!c.options.force and db.hasIntegrity(&package.integrity)) {
            cache_hits +|= 1;
            continue;
        }

        const cache_path = db.getPackagePath(
            &package.integrity,
            arena_alloc,
        ) catch return .cache_error;
        storage.remove(cache_context, cache_path, true) catch {};
        const ext = extractor.StorageExtractor.init(
            c.allocator,
            cache_context,
            cache_path,
        ) catch |err| {
            c.setErrorFmt("Failed to prepare package cache: {}", .{err});
            return .extract_error;
        };
        const extract_context = arena_alloc.create(PortableExtractContext) catch {
            ext.deinit();
            return .out_of_memory;
        };
        extract_context.* = .{
            ...
            .cache_path = cache_path,
            .package_name = package.name.slice(),
            .version = package.version.format(arena_alloc) catch {
                ext.deinit();
                return .out_of_memory;
            },
        };
        extract_contexts.append(arena_alloc, extract_context) catch {
            ext.deinit();
            return .out_of_memory;
        };

        http.fetchTarball(package.tarball_url, fetcher.StreamHandler.init(
            struct {
                fn onData(data: []const u8, user_data: ?*anyopaque) void {
                    const context: *PortableExtractContext = @ptrCast(@alignCast(user_data));
                    if (context.failed) return;
                    context.ext.feedCompressed(data) catch |err| {
                        context.failed = true;
                        context.failure = @errorName(err);
                    };
                }
            }.onData,
            struct {
                fn onComplete(status: u16, _: ?[]const u8, user_data: ?*anyopaque) void {
                    const context: *PortableExtractContext = @ptrCast(@alignCast(user_data));
                    if (status != 200) context.failed = true;
                    context.completed = true;
                }
            }.onComplete,
            struct {
                fn onError(err: fetcher.FetchError, user_data: ?*anyopaque) void {
                    const context: *PortableExtractContext = @ptrCast(@alignCast(user_data));
                    context.failed = true;
                    context.completed = true;
                    context.failure = @errorName(err);
            ...
            extract_context.failed = true;
            extract_context.completed = true;
        };
    }

    if (extract_contexts.items.len > 0) {
        http.run() catch {
            if (c.isCancelled()) {
                c.setError("Dependency installation was cancelled");
                for (extract_contexts.items) |context| context.ext.deinit();
                return .cancelled;
            }
            c.setError("Failed while downloading dependency tarballs");
            for (extract_contexts.items) |context| context.ext.deinit();
            return .network_error;
        };
    }

    for (extract_contexts.items, 0..) |context, index| {
        c.checkCancelled() catch {
            c.setError("Dependency installation was cancelled");
            return .cancelled;
        };
        defer context.ext.deinit();
        if (context.failed or !context.completed or !context.ext.isComplete()) {
            storage.remove(cache_context, context.cache_path, true) catch {};
            const reason = if (context.failure.len > 0) context.failure else "incomplete tarball";
            c.setErrorFmt("Failed to download or extract {s}: {s}", .{ context.package_name, reason });
            return .extract_error;
        }
        ...
        c.reportProgress(
            .caching,
            @intCast(index + 1),
            @intCast(extract_contexts.items.len),
            message,
        );
        const stats = context.ext.stats();
        db.insert(&.{
            ...
            .file_count = stats.files,
            .cached_at = std.Io.Timestamp.now(io, .real).toSeconds(),
        }, context.package_name, context.version) catch |err| {
            c.setErrorFmt("Failed to update dependency cache: {}", .{err});
            return .cache_error;
        };
    }

    var package_linker = linker.StorageLinker.init(
        c.allocator,
        cache_context,
        project,
    );
    defer package_linker.deinit();
    package_linker.setNodeModulesPath(node_modules_relative) catch |err| {
        c.setErrorFmt("Failed to prepare project node_modules: {}", .{err});
        return .io_error;
    };

    // .bin and .ant are linker-owned state. Clear stale entries before linking
    // so the current install can recreate the links and install marker.
    const bin_relative = storage.join(arena_alloc, node_modules_relative, ".bin") catch
        return .out_of_memory;
    const virtual_relative = storage.join(arena_alloc, node_modules_relative, ".ant") catch
        return .out_of_memory;
    storage.remove(project, bin_relative, true) catch {};
    storage.remove(project, virtual_relative, true) catch {};
    storage.mkdirs(project, bin_relative) catch |err| {
        c.setErrorFmt("Failed to prepare project .bin directory: {}", .{err});
        return .io_error;
    };

    var links = std.ArrayListUnmanaged(linker.PackageLink).empty;
    defer links.deinit(arena_alloc);
    package_iter = res.resolved.valueIterator();
    while (package_iter.next()) |package_ptr| {
        const package = package_ptr.*;
        var entry = db.lookup(&package.integrity) orelse {
            c.setErrorFmt("Dependency cache entry is missing for {s}", .{package.name.slice()});
            return .cache_error;
        };
        defer entry.deinit();
        package.file_count = entry.file_count;
        links.append(arena_alloc, .{
            .cache_path = arena_alloc.dupe(u8, entry.path) catch return .out_of_memory,
            .node_modules_path = node_modules_relative,
            .name = package.name.slice(),
            .parent_path = package.parent_path,
            .file_count = entry.file_count,
            .has_bin = package.has_bin,
            .bins = resolvedPackageBins(arena_alloc, package) catch
                return .out_of_memory,
        }) catch return .out_of_memory;
    }
    std.sort.heap(
        ...
        links.items,
        {},
        packageLinkLessThanParentFirst,
    );
    for (links.items, 0..) |link, index| {
        ...
        c.reportProgress(.linking, @intCast(index + 1), @intCast(links.items.len), message);
        package_linker.linkPackage(link) catch |err| {
            c.setErrorFmt("Failed to install {s}: {}", .{ link.name, err });
            return .io_error;
        };
    }

    // Linking is complete and the new graph is known to be valid. Remove only
    // top-level package directories that are no longer part of that graph; the
    // shared cache remains untouched and can still serve other projects.
    _ = pruneStaleTopLevelPackages(
        project,
        node_modules_relative,
        &res,
        arena_alloc,
    ) catch |err| {
        c.setErrorFmt("Failed to prune removed dependencies: {}", .{err});
        return .io_error;
    };
    if (c.options.cache_is_project_storage) {
        _ = pruneUnreferencedProjectCache(db, cache_context, &res, arena_alloc) catch |err| {
            c.setErrorFmt("Failed to prune project dependency cache: {}", .{err});
            return .cache_error;
        };
    }

    const graph_hash = res.writeLockfileStorage(
        project,
        lockfile_relative,
    ) catch |err| {
        c.setErrorFmt("Failed to write project ant.lockb: {}", .{err});
        return .io_error;
    };
    const marker_path = storage.join(
        arena_alloc,
        node_modules_relative,
        ".ant/install-state",
    ) catch return .out_of_memory;
    const marker = portableInstallStateMarkerContent(
        arena_alloc,
        graph_hash,
        package_json_hash,
    ) catch
        return .out_of_memory;
    storage.atomicWrite(project, marker_path, marker, arena_alloc) catch |err| {
        c.setErrorFmt("Failed to record install state: {}", .{err});
        return .io_error;
    };
    db.sync();

    const link_stats = package_linker.getStats();
    c.last_install_result = .{
        .package_count = @intCast(res.resolved.count()),
        .cache_hits = cache_hits,
        .cache_misses = @intCast(extract_contexts.items.len),
        .files_linked = link_stats.files_linked,
        .files_copied = link_stats.files_copied,
        .packages_installed = link_stats.packages_installed,
        .packages_skipped = link_stats.packages_skipped,
...
export fn pkg_resolve_and_install(
    ...
    const c = ctx orelse return .invalid_argument;
    const project_context = storage.fromOpaque(c.options.project_storage);
    const cache_context = storage.fromOpaque(c.options.cache_storage);
    if (project_context != null or cache_context != null) {
        if (project_context == null or cache_context == null) {
            c.setError("StorageLocation installs require both project and cache contexts");
            return .invalid_argument;
        }
        return portableResolveAndInstall(
            c,
            project_context.?,
            cache_context.?,
            std.mem.span(package_json_path),
    ...
    var pkg_linker = linker.Linker.init(c.allocator);
    defer pkg_linker.deinit();
    // The executable link directory and virtual install state are owned by the
    // current graph. Remove stale entries before recreating links so deleted
    // dependencies cannot remain invokable through node_modules/.bin.
    const bin_path = std.fmt.allocPrint(arena_alloc, "{s}/.bin", .{std.mem.span(node_modules_path)}) catch
        return .out_of_memory;
    const virtual_path = std.fmt.allocPrint(arena_alloc, "{s}/.ant", .{std.mem.span(node_modules_path)}) catch
        return .out_of_memory;
    std.Io.Dir.cwd().deleteTree(io, bin_path) catch {};
    std.Io.Dir.cwd().deleteTree(io, virtual_path) catch {};
    ...
    while (pkg_iter.next()) |pkg_ptr| {
        ...
        if (interleaved.queued_integrities.contains(pkg.integrity)) {
            ...
            for (interleaved.extract_contexts.items) |ext_ctx| {
                if (std.mem.eql(u8, &ext_ctx.integrity, &pkg.integrity)) {
                    is_download = true;
                    break;
                }
            }
    ...
    if (interleaved.tarballs_queued > 0) {
        ...
        tarball_thread = std.Thread.spawn(.{}, struct {
            fn work(h: *fetcher.Fetcher) void {
                h.run() catch {};
            }
    ...
    for (interleaved.extract_contexts.items) |ext_ctx| {
        if (ext_ctx.has_error or !ext_ctx.completed or !ext_ctx.ext.isComplete()) {
            ...
            debug.log("  error: {s}", .{ext_ctx.pkg_name});
            continue;
        }
        ...
        if (ext_ctx.staging_path) |staging| {
            ...
            const to_z = arena_alloc.dupeZ(u8, ext_ctx.cache_path) catch {
                error_count += 1;
                continue;
            };
            ...
            if (std.c.rename(from_z.ptr, to_z.ptr) != 0) {
                ...
                std.Io.Dir.cwd().deleteTree(io, staging) catch {};
                error_count += 1;
                debug.log("  error: {s} (force swap failed)", .{ext_ctx.pkg_name});
                continue;
            }
        }
    ...
    var max_link_ms = std.atomic.Value(usize).init(0);
                    ...
                    fn work(lnk: *linker.Linker, jobs: []const LinkJobWithSize, next: *std.atomic.Value(usize), pkg_ctx: *PkgContext, total: u32, counter: *std.atomic.Value(u32), link_errors: *std.atomic.Value(u32), slow_count: *std.atomic.Value(u32), max_ms: *std.atomic.Value(usize), names: *std.ArrayListUnmanaged([]const u8), lock: *std.Io.Mutex, alloc: std.mem.Allocator) void {
                        while (true) {
                            ...
                            if (elapsed_ms > 100) {
                                ...
                                lock.unlock(io);
                                const elapsed_ms_native = std.math.cast(usize, elapsed_ms) orelse std.math.maxInt(usize);
                                var current_max = max_ms.load(.monotonic);
                                while (elapsed_ms_native > current_max) : (current_max = max_ms.load(.monotonic)) {
                                    if (max_ms.cmpxchgWeak(current_max, elapsed_ms_native, .monotonic, .monotonic) == null) break;
                                }
                            }
                        }
                    }
    ...
    _ = pruneStaleTopLevelPackagesFs(
        std.mem.span(node_modules_path),
        &res,
        arena_alloc,
    ) catch |err| {
...
export fn pkg_add(
    ...
    const content = readProjectData(c, arena_alloc, pkg_json_str) catch |err| blk: {
        if (err == error.NotFound or err == error.FileNotFound) break :blk "{}";
        c.setErrorFmt("Failed to read package.json: {}", .{err});
        return .io_error;
    };
    ...
    const output = writer.writeData(arena_alloc) catch {
        c.setError("Failed to serialize package.json");
        return .io_error;
    };
    writeProjectData(c, arena_alloc, pkg_json_str, output) catch {
...
export fn pkg_add_many(
    ...
    const output = writer.writeData(arena_alloc) catch return .out_of_memory;
    writeProjectData(c, arena_alloc, pkg_json_str, output) catch {
        c.setError("Failed to write package.json");
        return .io_error;
    };
...
fn jsonValueToMut(writer: *json.JsonWriter, value: std.json.Value) !*json.MutableValue {
    return switch (value) {
...
export fn pkg_remove(
    ...
    const content = readProjectData(c, arena_alloc, pkg_json_str) catch |err| {
        c.setErrorFmt("Failed to read package.json: {}", .{err});
        return .io_error;
    };
    ...
    const dep_keys = [_][]const u8{ "dependencies", "devDependencies", "peerDependencies", "optionalDependencies" };
...
export fn pkg_list_package_bins(
    ...
    if (root_val.getObject("bin")) |bin_obj| {
        ...
        const simple_name = if (std.mem.indexOf(u8, pkg_name, "/")) |slash| pkg_name[slash + 1 ..] else pkg_name;
...
export fn pkg_exec_temp(
    ...
    if (isAntsLandRegistry(configured_registry)) {
        ...
        if (choice == .fallback) {
            ...
            fallback_http = fetcher.Fetcher.initWithConnections(
                c.allocator,
                effective_registry,
                c.options.max_connections,
    ...
    if (c.options.run_lifecycle_scripts and !runTrustedPostinstall(c, &trusted, temp_nm_dir, arena_alloc)) return .io_error;
