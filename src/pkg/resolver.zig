...
pub const Resolver = struct {
  ...
  pub fn resolveFromPackageJson(self: *Resolver, path: []const u8) !void {
    ...
    try self.resolveFromPackageJsonParsed(&pkg_json);
  }

  pub fn resolveFromPackageJsonData(self: *Resolver, data: []const u8) !void {
    var pkg_json = try json.PackageJson.parseData(self.allocator, data);
    defer pkg_json.deinit(self.allocator);
    try self.resolveFromPackageJsonParsed(&pkg_json);
  }

  fn resolveFromPackageJsonParsed(self: *Resolver, pkg_json: *const json.PackageJson) !void {
  ...
  pub fn writeLockfile(self: *Resolver, path: []const u8) !u64 {
    return self.writeLockfileTo(.{ .file_path = path });
  }

  pub fn writeLockfileStorage(
    self: *Resolver,
    context: *const storage.Context,
    path: []const u8,
  ) !u64 {
    return self.writeLockfileTo(.{ .storage = .{
      .context = context,
      .path = path,
    } });
  }

  const LockfileDestination = union(enum) {
    file_path: []const u8,
    storage: struct {
      context: *const storage.Context,
      path: []const u8,
  ...
  fn writeLockfileTo(self: *Resolver, destination: LockfileDestination) !u64 {
    ...
    return switch (destination) {
      .file_path => |path| try writer.write(path),
      .storage => |target| try writer.writeStorage(target.context, target.path),
    };
  }
};
