...
const portable_cache = @import("portable_cache.zig");
...
pub const CacheDB = struct {
  ...
  pub fn openStorage(context: *const storage.Context) !*CacheDB {
    const allocator = std.heap.c_allocator;
    const portable = try portable_cache.PortableCache.open(context);
    errdefer portable.close();
    const self = try allocator.create(CacheDB);
    self.* = .{
      .env = null,
      .dbi_primary = 0,
      .dbi_secondary = 0,
      .dbi_metadata = 0,
      .cache_dir = try allocator.dupe(u8, ""),
      .allocator = allocator,
      .portable = portable,
    };
    return self;
  ...
  fn openDatabases(self: *CacheDB) !void {
    const env = self.env orelse return error.DatabaseOpen;
  ...
  pub fn close(self: *CacheDB) void {
    if (self.portable) |portable| portable.close() else if (self.env) |env| {
      c.mdb_dbi_close(env, self.dbi_primary);
      c.mdb_dbi_close(env, self.dbi_secondary);
      c.mdb_env_close(env);
    }
  ...
  pub fn lookup(self: *CacheDB, integrity: *const [64]u8) ?CacheEntry {
    if (self.portable) |portable| {
      const entry = portable.lookup(integrity) orelse return null;
      return .{
    ...
    if (c.mdb_txn_begin(env, null, c.MDB_RDONLY, &txn) != 0) {
      return null;
  ...
  pub fn hasIntegrity(self: *CacheDB, integrity: *const [64]u8) bool {
    if (self.portable) |portable| return portable.hasIntegrity(integrity);
    var txn: ?*c.MDB_txn = null;
  ...
  pub fn lookupByName(self: *CacheDB, name: []const u8, version: []const u8) ?CacheEntry {
    if (self.portable) |portable| {
      const entry = portable.lookupByName(name, version) orelse return null;
      return .{
        .integrity = entry.integrity,
        .path = entry.path,
        .unpacked_size = entry.unpacked_size,
  ...
  pub fn batchLookup(
    ...
    if (self.portable) |portable| {
      ...
      for (integrities, 0..) |integrity, index| {
        if (portable.lookup(&integrity)) |entry| {
          try hits.append(allocator, .{ .index = @intCast(index), .file_count = entry.file_count });
          var mutable = entry;
          mutable.deinit();
        }
      }
      return .{ .items = try hits.toOwnedSlice(allocator), .allocator = allocator };
    }
    var hits = std.ArrayListUnmanaged(BatchHit).empty;
    errdefer hits.deinit(allocator);

    var txn: ?*c.MDB_txn = null;
    const env = self.env orelse return error.DatabaseError;
    if (c.mdb_txn_begin(env, null, c.MDB_RDONLY, &txn) != 0) {
  ...
  pub fn insert(self: *CacheDB, entry: *const CacheEntry, name: ?[]const u8, version: ?[]const u8) !void {
    if (self.portable) |portable| return portable.insert(entry, name, version);
    var txn: ?*c.MDB_txn = null;
    const env = self.env orelse return error.InsertError;
    if (c.mdb_txn_begin(env, null, 0, &txn) != 0) {
  ...
  pub fn batchInsert(self: *CacheDB, entries: []const CacheEntry) !void {
    if (self.portable) |portable| {
      for (entries) |*entry| try portable.insert(entry, null, null);
      return;
    }
    var txn: ?*c.MDB_txn = null;
    const env = self.env orelse return error.InsertError;
  ...
  pub fn batchInsertNamed(self: *CacheDB, entries: []const NamedCacheEntry) !void {
    ...
    if (self.portable) |portable| {
      for (entries) |item| try portable.insert(&item.entry, item.name, item.version);
      return;
    }
  ...
  pub fn getPackagePath(self: *CacheDB, integrity: *const [64]u8, allocator: std.mem.Allocator) ![]u8 {
    if (self.portable) |portable| return portable.getPackagePath(integrity, allocator);
  ...
  pub fn sync(self: *CacheDB) void {
    if (self.portable) |_| return;
    if (self.env) |env| _ = c.mdb_env_sync(env, 1);
  }

  pub fn stats(self: *CacheDB) !Stats {
    if (self.portable) |portable| {
      const result = try portable.stats();
      return .{
  ...
  pub fn lookupMetadata(self: *CacheDB, registry_host: []const u8, name: []const u8, allocator: std.mem.Allocator) ?[]u8 {
    if (self.portable) |portable| return portable.lookupMetadata(registry_host, name, allocator);
    var txn: ?*c.MDB_txn = null;
    const env = self.env orelse return null;
    if (c.mdb_txn_begin(env, null, c.MDB_RDONLY, &txn) != 0) {
      return null;
  ...
  pub fn insertMetadata(self: *CacheDB, registry_host: []const u8, name: []const u8, json_data: []const u8) !void {
    ...
    if (self.portable) |portable| {
      return portable.insertMetadata(registry_host, name, data_to_store);
    }
  ...
  pub fn prune(self: *CacheDB, max_age_days: u32) !u32 {
    if (self.portable) |portable| return portable.prune(max_age_days);
    const now = std.Io.Timestamp.now(io, .real).toSeconds();
    ...
    var txn: ?*c.MDB_txn = null;
    const env = self.env orelse return error.DatabaseError;
    if (c.mdb_txn_begin(env, null, 0, &txn) != 0) return error.DatabaseError;
    errdefer c.mdb_txn_abort(txn);
