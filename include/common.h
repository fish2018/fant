...
/* Android's AArch64 allocator may return a logically ordinary pointer with a
 * top-byte allocation tag (TBI/MTE). NaN-boxed values only retain the low
 * 47 address bits, so remove that non-address tag for range validation. The
 * original pointer is returned unchanged so the platform allocator keeps its
 * tag semantics for native accesses and free(). */
static inline uintptr_t ant_pointer_value(const void *p) {
