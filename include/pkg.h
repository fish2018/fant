...
#define PKG_STORAGE_FILE_PATH ANT_STORAGE_FILE_PATH
#define PKG_STORAGE_SAF_TREE ANT_STORAGE_SAF_TREE

typedef struct {
  const char *cache_dir;
  pkg_storage_location_t cache_location;
  const pkg_storage_bridge_t *cache_bridge;
  /* Opaque contexts are created by the embedding host with
   * ant_storage_context_create(). They are the authoritative backend for
